...
namespace TopFilms.Controllers
{
    ...
    public class ActorController : ControllerBase
    {
        ...
        //GET api/actor
        [HttpGet]
        public ActionResult<IEnumerable<ActorReadDto>> GetAll()
        {
            var commandItems = _repository.GetAll();

            return Ok(_mapper.Map<IEnumerable<ActorReadDto>>(commandItems));
        }

        //GET api/actor/{id}
        [HttpGet("{id}", Name = "GetId")]
        public ActionResult<ActorReadDto> GetId(int id)
        {
            var commandItem = _repository.GetId(id);
            if (commandItem != null)
            {
                return Ok(_mapper.Map<ActorReadDto>(commandItem));
            }
            return NotFound();
        }
        ...
        //POST api/actor/{id}
        [HttpPost]
        public ActionResult<ActorReadDto> Create(ActorCreateDto actorCreateDto)
        {
            var commandModel = _mapper.Map<Actor>(actorCreateDto);
            _repository.Create(commandModel);
            _repository.SaveChanges();

            var actorReadDto = _mapper.Map<ActorReadDto>(commandModel);
        ...
        //PUT api/actor/{id}
        [HttpPut("{id}")]
        public ActionResult Update(int id, ActorUpdateDto actorUpdateDto)
        {
        ...
        //PATCH api/actor/{id}
        [HttpPatch("{id}")]
        public ActionResult PartialUpdate(int id, JsonPatchDocument<ActorUpdateDto> patchDoc)
        {
            var commandModelFromRepo = _repository.GetId(id);
            if (commandModelFromRepo == null)
            {
            ...
            var commandToPatch = _mapper.Map<ActorUpdateDto>(commandModelFromRepo);
